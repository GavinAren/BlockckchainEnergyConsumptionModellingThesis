...
Paper \cite{Powell2021AWARENESSBLOCKCHAIN} by Powell et al. aims to make recommendations for building greener blockchains. It applied pre-existing formulae from \cite{CambridgeCBECI} to calculate the energy consumption of Bitcoin and Ethereum 1.0, both of which employ PoW consensus. It did come up with a simple formula to calculate the energy 
usage of the PoS consensus mechanism of the Polkadot blockchain: “Polkadot Energy Consumption = Energy per server * number of servers * 24 hours * 365 days” [27]. This equation could be included as a small part of the larger mathematical model this paper will create. Still, all research reviewed so far is too focused on PoW blockchains and specifically in the context of cryptocurrency. 

A recent paper by UCL Centre for Blockchain Technologies \cite{PlattDiscussionProof-of-Work} successfully found a good mathematical relationship between validator number, load and hardware configuration when measuring the energy consumption of the PoS consensus protocol in isolation. It combines past literature, datasets from various sources, including the CCRI report \cite{CryptoCarbonRatingsInstitute2022TheNetwork} and public ledger information to avoid time-consuming investigation and experimentation, just like this paper aims to do. It builds upon the simple equation for PoS suggested by the last paper \cite{Powell2021AWARENESSBLOCKCHAIN}. 

While its results are good, the paper only took validator nodes that are actively participating in the consensus protocol into account for its model, as it only aimed to model the consensus protocol’s energy usage in isolation. Thus, incomplete in its calculation of network-wide energy consumption by ignoring factors such as the 'Full Nodes' that aren't actively participating in validation transactions but just contribute to securing the blockchain, which is included in the model proposed in this report. It also relies on speculative assumptions as it was written before Ethereum’s upgrade to PoS consensus (now the world’s largest PoS blockchain). Hence, the most significant PoS blockchain was not included in its study.
