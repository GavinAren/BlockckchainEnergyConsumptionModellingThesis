...
To capture only the energy consumption of the syncing process, we assume the CPU to be operating at its maximum capacity, hence taking $\boldsymbol{\mathrm{U}}$ to equal 100\%, or $\boldsymbol{1}$ for the sake of simplicity. This negates the need for $\boldsymbol{\mathrm{P}_{idle}}$ to be included in the equation, leaving:
...
\label{DetermineK}
Below is an example calculation to determine constant $\boldsymbol{\mathrm{k}}$ for a node with hardware configuration 6 (see in \fref{Figure:CCRIhardwareConfig}, \sref{ImplementationSection}). 

$\boldsymbol{\mathrm{C}} = \boldsymbol{\mathrm{150.20}} $W, assuming the node runs the most common CL and EL client combination, Prysm and Geth. Mean values are taken from  \cite{CryptoCarbonRatingsInstitute2022TheNetwork}.
...
\end{tabular}
\caption{Mean electricity consumption values [in Wh] measured in the report \cite{CryptoCarbonRatingsInstitute2022TheNetwork} for hardware configurations 4-6 mentioned in \fref{Figure:CCRIhardwareConfig} running various clients.  }
\label{Table:ConsumptionValues}
\end{table}

Given the data in \tref{Table:ConsumptionValues}, values in \tref{Table:EstimatesPerNode} can be calculated. For example, the first row (Prysm,Geth) was calculated by adding the electricity usage when idle, running Pyrsm, and running Geth per configuration, multiplied by its hardware tier distribution across the network.
\begin{align}
...
% -----------------------------------
Following \eref{eqn:CCRI}, best guess for an individual node =  \textbf{63.76 Wh}. This can be annualised (assuming 8760h in a year) = \textbf{558.55 kWh/year}.

Assuming the node needs to perform a fresh synchronisation once a year, the annual total consumption of an average node (running 1 validator):
...
To account for some nodes are running multiple validators, the final part of \eref{eqn:FinalEqnShort}, $\boldsymbol{\mathrm{\overline{V}(\mathrm{x})}}$ must be calculated. 

To reiterate \sref{AdditonalNodesReasoning}, mid-tier nodes can run upto 1000 nodes while high-tier nodes can run 2500+ nodes \cite{2021HardwareEthstaker} \cite{Kaushal2022ValidatingConference}. When users run additional validator nodes, they often run a minimum of 1000+ validators, leveraging the exponential decrease in electricity consumption discussed before, to their advantage. Since there are currently 561,472 validators, for the sake of simplicity, it is assumed that 1000 additional validators are run on $\sim$562 nodes (of the total $\sim$12000 network nodes), and are running on mid to high-tier hardware. Following \eref{eqn:Sync}:
...
\begin{center}
    $\boldsymbol{\mathrm{\overline{V}(\mathrm{1000})}} =$ \textbf{141.1 Wh}
\end{center}
...
Final results can be found in \tref{Table:FinalResults}.
...
Metrics as of 10$\mathrm{^{th}}$ April 23: \\
...
Results calculated using these metrics for each of the aforementioned models are presented in \tref{Table:FinalResults}.
\clearpage
% -----------------------------------------------
% RESULTS  TABLEEEE
% ----------------------------------------------------------------
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Trusted sources well justify each conclusion or assumption drawn during the formulation of Model A and have also been well documented (\sref{Assumptions}).
