...
\label{eqn:FinalEqnLong}
     &\boldsymbol{({\mathrm{T}_{SNC} * ({\mathrm{P}_{TDP}} + \mathrm{P}_{SSD})}) +  {\frac{\displaystyle\sum\limits_{i=1}^{n}{ \left({\left(\mathrm{\overline{P}}_{ID} + \mathrm{\overline{P}}_{CL} + \mathrm{\overline{P}}_{EL}\right)} * {\phi_{EL,CL}} \right)}}
{\pi}}}\nonumber \\  \nonumber\\  
     &\boldsymbol{+ {(\mathrm{P}_{TDP} - {\frac{\displaystyle\sum\limits_{i=1}^{n}{ \left({\left(\mathrm{\overline{P}}_{ID} + \mathrm{\overline{P}}_{CL} + \mathrm{\overline{P}}_{EL}\right)} * {\phi_{EL,CL}} \right)}}
...
\textbf{Implementing CCRI model with Updated Data }
The base model proposed by CCRI \cite{CryptoCarbonRatingsInstitute2022TheNetwork} found in equation \ref{eqn:CCRI} in section \ref{CCRIBaseEqnSection} had the EL and CL client shares on the network. Table \ref{Table:ClientShares} below has updated values for these. 
...
OpenEthereum has been deprecated and Lodestar no longer holds a sizeable share of the network and they are not be included in the calculations. The Nethermind client which now holds a sizeable share of the network has no recorded electricity consumption data, hence an average value per configuration between the other 3 EL clients was used. 

Next, data on mean values recorded for all 3 configurations running all clients was extracted and collected in \tref{Table:ConsumptionValues}. Using the same assumptions made in the CCRI report, this \tref{Table:EstimatesPerNode} has been recalculated with updated values. 
...
\multicolumn{2}{|l|}{\textbf{Client Combination}} &
  \multirow{2}{*}{\textbf{\begin{tabular}[c]{@{}l@{}}Estimate\\ {[}W{]}\end{tabular}}} &
...
\end{tabular}
\caption{Electricity consumption estimates per node for all possible client combinations along with their network share. Individual client shares found in \tref{Table:ClientShares}  }
...
Best guess for an individual node [W]: 63.7613088
